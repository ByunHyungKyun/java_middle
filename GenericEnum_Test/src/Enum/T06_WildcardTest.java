...
public class T06_WildcardTest {
	...
	Juicer.makeJuice(appleBox); //주스클래스에서 제네릭을 이용하여 오류를 없앴다.원래는 과일박스만 허용한다	
	}
}

class Juicer {
	//static void makeJuice(FruitBox<Fruit> box) {
		//static <T> void makeJuice(FruitBox<T> box) { //제네릭 메서드를 사용한 예
			static void makeJuice(FruitBox<? extends Fruit> box) {//와일드 카드를 이용한 일반 메서드
