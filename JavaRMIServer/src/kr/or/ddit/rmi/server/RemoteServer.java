...
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
	...
	protected RemoteServer() throws RemoteException {//객체를 만드는 순간에도 예외가 발생할수있으니 만들으라는 뜻이다.
		super();
	}
	...
	public static void main(String[] args) {
		try {
			...
			//구현한RMI용 객체를 클라이언트에서 사용할수 있도록 
			//RMI서버에 등록한다
			
			//1.RMI용 인터페이스를 구현한 객체생성
	...
	public int doRemotePrint(String str) throws RemoteException {
		...
		System.out.println("클라이언드에서 보내온 메세지: "+str);
	...
	public void setFiles(FileInfoVO[] fInfo) throws RemoteException{
		...
		for (FileInfoVO fvo : fInfo) {
			try {
				...
				//클라리언트에서 전달한 파일 데이터(byte[])를 서버측에 저장한다.
