...
/*
 * 클라이언트쪽으로 프로젝트에도 서버의 패키지와 같은 구조로 
...
public class RemoteClient {
	public static void main(String[] args) {
		...
		try {
			...
			//이제부터는 불러온 객체의 메서드를 호출해서 사용할수 있다
			int a = clientInf.doRemotePrint("안녕하세요");//내가 보낸 값을 내가 확인할수있게 다시 받는다
			System.out.println("반환값=> "+a);
			System.out.println("--------------------------------");
			
			List<String> list = new ArrayList<String>();//인터페이스에서 직렬화를 선언해줬다
			list.add("대전");
			list.add("대구");
			list.add("광주");
			list.add("서울");
			...
			System.out.println("VO 출력메서드 호출 끝...");
			System.out.println("-----------------------------------");
			
			//파일 전송하기
			File[] files = new File[2];
			files[0] = new File("d:/D_Other/다운로드4.jpg");
			files[1] = new File("d:/D_Other/다운로드1.jpg");
